...
def read_questions(xlsx_path: str) -> list[str]:
    ...
    # First pass: try the legacy "numbered prefix" format (e.g. "1. What…").
    # If we find ZERO numbered rows, fall back to a plain pass that treats
    # every non-empty cell as a question. This keeps backward-compat with
    # the original auto-numbered xlsx while supporting the newer
    # `test_questions_for_all_projects.xlsx` (plain text rows, no numbering).
    ...
    if not questions:
        # Fallback — accept any non-empty row that looks like a question
        # (heuristic: contains at least 3 words OR ends with "?").
        for row in rows:
            raw = str(row[0] if row[0] else "").strip()
            if not raw:
                continue
            if len(raw) < 8:
                continue
            if raw.endswith("?") or len(raw.split()) >= 3:
...
def _parse_response(question: str, data: dict, latency: float) -> dict:
    ...
    v3 (Tier 1+2+3 citation patches) adds:
      - source_documents[].bbox_pt / text_excerpt   (Tier 1)
      - source_documents ranked by answer-mention   (Tier 2)
      - inline_citations[]                           (Tier 3)
    Older endpoints (port 8001) won't have these fields populated; the
    extractor below tolerates that and emits empty values.
    """
    source_docs = data.get("source_documents") or []
    source_titles = "; ".join(
        d.get("display_title") or d.get("drawing_name") or d.get("file_name") or "N/A"
        for d in source_docs[:5]
    )
    pdf_names = "; ".join(
        d.get("file_name") or d.get("pdf_name") or "N/A" for d in source_docs[:5]
    )
    s3_paths = "; ".join(data.get("s3_paths", [])[:5])

    # v3 — top-1 source identity (most-cited drawing)
    top = source_docs[0] if source_docs else {}
    top_drawing = top.get("drawing_name") or ""
    top_page = top.get("page") or ""
    top_has_bbox = bool(top.get("bbox_pt"))
    top_text_excerpt = (top.get("text_excerpt") or "")[:200]

    # v3 — bbox / text_excerpt coverage across the whole source list
    src_with_bbox = sum(1 for d in source_docs if d.get("bbox_pt"))
    src_with_text = sum(1 for d in source_docs if d.get("text_excerpt"))

    # v3 — inline citations parsed from the answer
    inline_cites = data.get("inline_citations") or []
    inline_cite_count = len(inline_cites)
    inline_cite_with_bbox = sum(1 for c in inline_cites if c.get("bbox_pt"))
    inline_cite_summary = "; ".join(
        f"[{c.get('drawing_name', '?')} p{c.get('page', '?')}]"
        + ("•bb" if c.get("bbox_pt") else "")
        for c in inline_cites[:6]
    )
    ...
    return {
        ...
        # ── v3 columns (empty against the old endpoint, populated on 8007) ──
        "source_count": len(source_docs),
        "top_source_drawing": top_drawing,
        "top_source_page": top_page,
        "top_source_has_bbox": top_has_bbox,
        "top_source_text_excerpt": top_text_excerpt,
        "sources_with_bbox": src_with_bbox,
        "sources_with_text_excerpt": src_with_text,
        "inline_citation_count": inline_cite_count,
        "inline_citations_with_bbox": inline_cite_with_bbox,
        "inline_citations_summary": inline_cite_summary,
        # ────────────────────────────────────────────────────────────────────
        "success": is_success,
        "error": error_text,
    }
...
def _error_result(question: str, latency: float, error: str) -> dict:
    return {
        ...
        "top_source_drawing": "",
        "top_source_page": "",
        "top_source_has_bbox": False,
        "top_source_text_excerpt": "",
        "sources_with_bbox": 0,
        "sources_with_text_excerpt": 0,
        "inline_citation_count": 0,
        "inline_citations_with_bbox": 0,
        "inline_citations_summary": "",
        "success": False,
        "error": error,
    }
...
CSV_FIELDS = [
    ...
    # ── v3 Tier 1+2+3 columns ──
    "source_count",
    "top_source_drawing", "top_source_page",
    "top_source_has_bbox", "top_source_text_excerpt",
    "sources_with_bbox", "sources_with_text_excerpt",
    "inline_citation_count", "inline_citations_with_bbox",
    "inline_citations_summary",
    # ───────────────────────────
    "success", "error",
]
