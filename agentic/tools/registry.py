...
# ---------------------------------------------------------------------------
# 8001 PROD — ENABLE_SPECIFICATIONS master gate (added 2026-06-02 per user request)
# ---------------------------------------------------------------------------
# When ENABLE_SPECIFICATIONS=false, ALL spec tools are removed from both
# TOOL_REGISTRY and TOOL_DEFINITIONS so the agent cannot retrieve from any
# spec source (legacy `specification` Mongo collection OR `specifications_v3`).
# This forces the agent to use drawings_v3 + legacy drawing tools only.
# Default = true (preserves current behavior). Flip to "false" in .env +
# restart rag-agent.service to disable; flip back to "true" + restart to
# re-enable. No code changes needed to toggle.
#
# Supersedes the earlier ENABLE_SPECIFICATIONS_V3 v3-only gate. The legacy
# flag is still honored for backwards compatibility: setting either
# ENABLE_SPECIFICATIONS=false OR ENABLE_SPECIFICATIONS_V3=false disables the
# six v3 tools; only ENABLE_SPECIFICATIONS controls the four legacy tools.
import os as _os


def _flag_off(name: str) -> bool:
    return _os.environ.get(name, "true").lower() in ("false", "0", "off", "no")


_SPECIFICATIONS_ENABLED = not _flag_off("ENABLE_SPECIFICATIONS")
_SPEC_V3_ENABLED = _SPECIFICATIONS_ENABLED and not _flag_off("ENABLE_SPECIFICATIONS_V3")

_SPEC_V3_TOOLS = {
    "search_specifications_v3",
    "v3_get_spec_by_csi",
    "v3_get_full_spec_section",
    "list_specifications_v3",
    "v3_get_spec_submittals",
    "v3_get_spec_warranties",
}
_SPEC_LEGACY_TOOLS = {
    "spec_list",
    "spec_search",
...
# [V19-SHELF] collection-mode support: spec tools disabled by the global gate are
# SHELVED (not discarded) so search_mode="specification" can use them per-request.
SPEC_TOOL_NAMES = _SPEC_V3_TOOLS | _SPEC_LEGACY_TOOLS
SHELVED_SPEC_REGISTRY: dict = {}
SHELVED_SPEC_DEFINITIONS: list = []


def _filter_disabled_tools():
    """Strip spec entries from TOOL_REGISTRY and TOOL_DEFINITIONS per env flags.
    Idempotent; safe to call once at import time after both structures are
    fully populated."""
    to_disable = set()
    if not _SPECIFICATIONS_ENABLED:
        to_disable |= _SPEC_V3_TOOLS | _SPEC_LEGACY_TOOLS
    elif not _SPEC_V3_ENABLED:
        to_disable |= _SPEC_V3_TOOLS
    if not to_disable:
        return
    # [V19-SHELF-MOVE] Move (not discard) so specification mode can restore per-request
    try:
        for name in list(TOOL_REGISTRY.keys()):
            if name in to_disable:
                SHELVED_SPEC_REGISTRY[name] = TOOL_REGISTRY.pop(name)
    except NameError:
        pass
    try:
        kept = []
        for d in TOOL_DEFINITIONS:
            ...
            if name in to_disable:
                SHELVED_SPEC_DEFINITIONS.append(d)
                continue
            kept.append(d)
        TOOL_DEFINITIONS.clear()
        TOOL_DEFINITIONS.extend(kept)
    except NameError:
        pass
    try:
        import logging as _logging
        _logging.getLogger(__name__).warning(
            "Spec tool gate active — disabled %d tools: %s. "
            "Agent will use drawings_v3 + legacy drawing tools only.",
            len(to_disable), sorted(to_disable),
        )
    except Exception:
        pass
