...
def search_specification_text(
    ...
    # v3.3 (2026-05-18): project parentId so downstream dedup can collapse
    # spec fragments that belong to the same logical section. The "_Rev_NN"
    # suffix in pdfName is NOT a revision — it is the chunk index of one
    # large spec PDF, and ALL chunks share the same parentId. Without this
    # field, source_documents shows up to 12 "different references" that
    # are actually slices of one document.
    try:
        results = list(coll.find(
            {"projectId": project_id, "$text": {"$search": search_text}},
            {
                ...
                "drawingTitle": 1, "drawingName": 1, "sectionText": 1,
        ...
        if results:
            ...
            return [{
                ...
                "drawingTitle": r.get("drawingTitle", ""),
                "drawingName": r.get("drawingName", ""),
...
def get_full_specification_text(
    ...
    * **Schema B** (e.g. projects 7222 / 7223): ``sectionTitle`` is ``None``,
    ...
    # v3.3 — carry parentId through so downstream dedup can recognize that
    # multiple "different pdfName" entries returned by other tools (e.g.
    # search_specification_text on a different code path) are all chunks of
    ...
    for p in parents:
        ...
        # Surface the same display fields the drawing tools surface so the
        # orchestrator's _extract_source_documents can populate display_title
        # / drawing_name / drawing_title for the UI. Without these the spec
        # source_documents come back with empty labels even though the data
        # exists in Mongo.
        results.append({
            "pdfName": p.get("pdfName", ""),
            "sectionTitle": title or "",
            "drawingTitle": p.get("drawingTitle") or title or "",
            "drawingName": p.get("drawingName") or "",
            "specificationNumber": p.get("specificationNumber", ""),
            "parentId": p.get("parentId"),
            "docNumber": p.get("docNumber"),
            "s3BucketPath": p.get("s3BucketPath", ""),
            # The full reconstructed text serves as the citation excerpt;
            # cap to 500 chars so it fits the UI hover tooltip without
            # bloating the response payload.
            "text": (full_text or "")[:500],
            "page": p.get("page_min"),
