...
# Phase P4: v3.1 generation chain. Imported eagerly because it's pure
# Python (no DB / SDK side-effects on import). Chain dispatch is gated
# by ``V31_CHAIN_ENABLED`` (default OFF) inside ``_maybe_run_v31_chain``.
from gateway.generation_chain import run_generation_chain, chain_enabled
...
def _extract_source_documents(
    ...
    for src in sources or []:
        if isinstance(src, dict):
            ...
            # Spec docs use sectionTitle + sectionText; drawing docs use
            # sheet_number + drawingTitle. Fall through both naming schemes
            # so ALL source types get a populated display_title (the field
            # the UI actually shows) rather than empty strings.
            display_title = (
                src.get("sheet_number")
                or src.get("name")
                or src.get("drawingTitle")        # rich human title
                or src.get("sectionTitle")        # CSI section name
                or src.get("drawingName")
                or src.get("specificationNumber")
                or ""
            )
            drawing_name = (
                src.get("drawingName")
                or src.get("drawing_name")
                or src.get("specificationNumber")  # spec docs: surface CSI number
                or ""
            )
            drawing_title = (
                src.get("drawingTitle")
                or src.get("drawing_title")
                or src.get("sectionTitle")         # spec docs: surface section name
                or ""
            )
            doc = {
                "s3_path": s3_path,
            ...
            # Tier 1 enrichment passthrough — when the agent's source_doc
            # carries bbox/text/score/csi/drawing_id/trade (because the agent
            # built it via _build_source_doc), surface those to the response.
            for tier1 in ("bbox_pt", "bbox_px", "text_excerpt", "csi_division",
                          "drawing_id", "drawingId", "score", "trade"):
                if tier1 in src and src[tier1] is not None:
                    out_key = "drawing_id" if tier1 == "drawingId" else tier1
                    doc[out_key] = src[tier1]
            # Spec sources additional passthrough — sectionText becomes
            # text_excerpt fallback so spec citations carry context.
            if not doc.get("text_excerpt"):
                doc["text_excerpt"] = (
                    src.get("text") or src.get("sectionText") or ""
                )[:500]
            # specificationNumber → csi_division fallback for spec sources
            if not doc.get("csi_division") and src.get("specificationNumber"):
                doc["csi_division"] = src.get("specificationNumber")
...
class Orchestrator:
    ...
    async def query(
        ...
        **kwargs: Any,
        ...
        # --- Phase P4: optional v3.1 generation chain (master flag OFF by default) ---
        # When V31_CHAIN_ENABLED=true, route through the new 6-agent chain.
        # When OFF, behaviour below is byte-identical to v3.0.
        if chain_enabled():
            chain_resp = await self._maybe_run_v31_chain(
                user_query=query,
                session_id=session_id,
                project_id=project_id,
                set_id=set_id,
    ...
    async def _maybe_run_v31_chain(
        self,
        *,
        user_query: str,
        session_id: Optional[str],
        ...
        """Phase P4: route through the v3.1 generation chain.

        Returns a complete UnifiedResponse-shaped dict, or ``None`` to
        signal "fall through to the legacy path" if the chain itself
        rejects (e.g., the agentic engine could not be initialised).

        Cache check: replicate the agent's existing cache lookup so a
        cached answer can flow through Branch A (re-expression) without
        rerunning ReAct. The lookup is cheap and read-only.
        """
        try:
            self.agentic.ensure_initialized()
        except Exception as exc:
            logger.warning(
                "v3.1 chain: agentic init failed (%s) — falling back to legacy",
                exc,
            )
            return None
        ...
        try:
            ...
            if cached_obj is not None:
                ...
                cached_dict = {
                    "answer": getattr(cached_obj, "answer", "") or "",
                    "session_id": session_id,
                    "engine_used": "agentic",
                    "source_documents": getattr(cached_obj, "source_docs", []) or [],
                    "sources": getattr(cached_obj, "sources", []) or [],
                    "confidence": getattr(cached_obj, "confidence", "medium"),
                    "model_used": getattr(cached_obj, "model", ""),
                }
        except Exception as exc:
            logger.debug("v3.1 chain: cache lookup skipped: %s", exc)

        try:
            chain_resp = await run_generation_chain(
            ...
            logger.exception("v3.1 chain crashed — falling back to legacy: %s", exc)
            return None

        # Merge chain response into the canonical _base_response shape so
        # the frontend gets every field it expects.
        resp = _base_response()
        resp.update(chain_resp)
        # Ensure the elapsed time is wall-clock from the orchestrator,
        # not the inner chain (which doesn't see DocQA / classifier work).
        resp["processing_time_ms"] = int((time.monotonic() - start) * 1000)
        resp["engine_used"] = "agentic"
        if scope:
            resp["scoped_to"] = scope.get("drawing_title") or scope.get("section_title")
        return resp
